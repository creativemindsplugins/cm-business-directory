...
if (!function_exists('cmbd_parse_php_info')) {

    function cmbd_parse_php_info() {
...
if (!function_exists('cmbd_file_exists_remote')) {
     ...
     * @return boolean
     */
    function cmbd_file_exists_remote($url) {
...
if (!function_exists('cmbd_sort_WP_posts_by_title_length')) {

    function cmbd_sort_WP_posts_by_title_length($a, $b) {
        $sortVal = 0;
...
if (!function_exists('cmbd_strip_only')) {
     ...
     * @return type
     */
    function cmbd_strip_only($str, $tags, $stripContent = false) {
        $content = '';
        if (!is_array($tags)) {
...
if (!function_exists('cmbd_truncate')) {
     ...
     * @return string
     */
    function cmbd_truncate($text, $length = 100, $ending = '...', $exact = false, $considerHtml = true) {
        if (is_array($ending)) {
            extract($ending);
        }
        if ($considerHtml) {
...
if (!function_exists('cmbd_show_message')) {
     ...
     */
    function cmbd_show_message($message, $errormsg = false) {
        if ($errormsg) {
...
if (!function_exists('cmbd_units2bytes')) {
...
if (!function_exists('cmbd_paginate_links')) {

    function cmbd_paginate_links($args = '') {
        $defaults = array(
        ...
        if ($prev_next && $current && 1 < $current) :
            ...
            if ($add_args)
                $link = esc_url(add_query_arg(urlencode_deep($add_args), $link));
            $link .= $add_fragment;
        ...
        if ($prev_next && $current && ( $current < $total || -1 == $total )) :
            ...
            /** This filter is documented in wp-includes/general-template.php */
            $page_links[] = '<a class="next page-numbers cmbd-button' . $link_class . '" href="' . esc_url(apply_filters('paginate_links', $link)) . '">' . $next_text . '</a>';
        endif;
        switch ($type) :
            case 'array' :
                return $page_links;
                break;
            case 'list' :
                $r .= '<ul class="page-numbers cmbd-button">\n\t<li>';
                $r .= join("</li>\n\t<li>", $page_links);
                $r .= "</li>\n</ul>\n";
                break;
            default :
